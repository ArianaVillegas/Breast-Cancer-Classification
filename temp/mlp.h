...
class MLP {
private:
    int n_layer, n_input, n_hidden, n_output;
    vector<Layer*> mlp;
...
public:
    MLP(int n_layer, int n_input, int n_hidden, int n_output){
        this->n_layer = n_layer;
        this->n_input = n_input;
        this->n_hidden = n_hidden;
        this->n_output = n_output;
        mlp.push_back(new Layer(n_input, n_hidden));
        for (int i = 1; i < n_layer; ++i) {
            mlp.push_back(new Layer(n_hidden, n_hidden));
        }
        mlp.push_back(new Layer(n_hidden, n_output));
    }
